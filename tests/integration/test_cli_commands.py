...
Tests individual CLI commands and their integration with services.
"""
...
class TestCLIIntegration:
    ...
    def test_cli_add_task_integration(self, setup_test_data, mock_input, mock_print, mock_clear_screen, mock_pause):
        """Test add_task CLI command integration"""
    ...
    def test_cli_view_tasks_integration(self, setup_test_data, mock_input, mock_print, mock_clear_screen, mock_pause):
        """Test view_tasks CLI command integration"""
        task_service = setup_test_data
    ...
    def test_cli_update_task_integration(self, setup_test_data, mock_input, mock_print, mock_clear_screen, mock_pause):
        """Test update_task CLI command integration"""
        task_service = setup_test_data
    ...
    def test_cli_mark_complete_integration(self, setup_test_data, mock_input, mock_print, mock_clear_screen, mock_pause):
        """Test mark_complete CLI command integration"""
        task_service = setup_test_data
    ...
    def test_cli_delete_task_integration(self, setup_test_data, mock_input, mock_print, mock_clear_screen, mock_pause):
        """Test delete_task CLI command integration"""
        task_service = setup_test_data
    ...
    def test_cli_command_error_handling(self, setup_test_data, mock_input, mock_print, mock_clear_screen, mock_pause):
    ...
    def test_cli_command_empty_input_handling(self, setup_test_data, mock_input, mock_print, mock_clear_screen, mock_pause):
        """Test CLI command handling of empty input"""
        task_service = setup_test_data
...
class TestCLIEdgeCases:
    ...
    def test_cli_with_special_characters(self, setup_test_data, mock_input, mock_print, mock_clear_screen, mock_pause):
        """Test CLI handling of special characters in task titles and descriptions"""
        task_service = setup_test_data
        ...
        mock_input.side_effect = [
            ...
            "1",  # Task ID to update
            "Updated «special» — task”",  # Updated title with special characters
            "6"   # Exit
        ]
        ...
        all_tasks = task_service.get_all_tasks()
        special_task = next((t for t in all_tasks if t.title == "Special Task !@#$%^&*()"), None)
        assert special_task is not None
        assert special_task.title == "Special Task !@#$%^&*()"

        updated_task = task_service.get_task_by_id(task_service.get_all_tasks()[0].id)
        assert updated_task.title == "Updated «special» — task”"
    ...
    def test_cli_with_long_titles_and_descriptions(self, setup_test_data, mock_input, mock_print, mock_clear_screen, mock_pause):
        ...
        mock_input.side_effect = [
            ...
            "1",  # Task ID to update
            "Updated " + long_title,  # Updated title
            "6"   # Exit
        ]
    ...
    def test_cli_with_unicode_characters(self, setup_test_data, mock_input, mock_print, mock_clear_screen, mock_pause):
        ...
        mock_input.side_effect = [
            ...
            "1",  # Task ID to update
            "更新后的任务",  # Updated title with Chinese characters
            "6"   # Exit
        ]
...
class TestCLIErrorScenarios:
    ...
    def test_cli_with_nonexistent_file(self):
        """Test CLI behavior when data file doesn't exist"""
        task_service = TaskService()
    ...
    def test_cli_with_corrupted_data(self, setup_test_data, mock_input, mock_print, mock_clear_screen, mock_pause):
        """Test CLI behavior with corrupted data file"""
        task_service = setup_test_data

        # Mock user input for viewing tasks
        mock_input.side_effect = ["2", "6"]  # View tasks, then exit
    ...
    def test_cli_with_permission_denied(self, setup_test_data, mock_input, mock_print, mock_clear_screen, mock_pause):
        """Test CLI behavior with permission denied error"""
        task_service = setup_test_data

        # Mock user input for adding a task
        mock_input.side_effect = ["1", "Test Task", "Description", "6"]
